...
Merge sequentially using `scripts/ci-merge-after-release.mjs <PR>`: it requires green checks and refuses while main CI, either release lane or landing deployment is queued/running. After a merge, wait for both releases and live verification before merging anything else, including docs/build changes. Never use `--admin` or auto-merge to sidestep that wait. Commit type does not establish safe supersession.
