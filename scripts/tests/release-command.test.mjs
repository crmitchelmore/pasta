...
test('release command preserves a GitHub response larger than 1 MiB', () => {
    const output = runReleaseCommand(process.execPath, ['-e',
        'process.stdout.write(JSON.stringify({body: "x".repeat(2 * 1024 * 1024)}))']);
    assert.equal(JSON.parse(output).body.length, 2 * 1024 * 1024);
});
