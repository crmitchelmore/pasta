...
- Bounded length-prefixed request/response protocol, explicit version, strict typed operations, no arbitrary path reads. Authenticated peers pull pending offers and chunks. Files use a validated manifest plus streamed chunks, with per-file digests and atomic staging. Constrain names, path depth, file count, total bytes, message sizes, connections and request lifetime; reject symlinks and special files rather than extracting arbitrary archives.
