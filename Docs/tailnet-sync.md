...
4. Run Swift build, full parallel tests including real service E2E, release-gate script tests, bundle readiness smoke and relevant UI checks. Per the user's 9 September delivery instruction, release after substantive PR review and required CI, then validate on their existing second tailnet Mac. Obtain that post-release evidence with synthetic payloads and separate app storage; never enable sharing of personal histories during testing.
...
- Sender snapshots require temporary free space for the payload plus a 64 MB reserve, with an aggregate 1 TB ceiling matching the maximum receiver setting. Captured history still references originals. The changed-since-capture notice uses modification time; deliberately backdated timestamps can evade that notice, while transfer approval remains bound to the payload hash.
...
- Full local Swift suite: 438 tests passed, including 33 tailnet-specific tests and a TCP capture-to-search-to-paste E2E. Local release-gate scripts: 27 tests passed. Landing contract tests and 12 Playwright tests passed after rebasing onto the website redesign.
...
- The initial Tailscale probe found both visible Mac peers offline. The user confirmed an existing second tailnet Mac and explicitly moved real two-Mac/Tailscale/Keychain/target-dialog validation to after release on 9 September. This physical test no longer blocks release; automated TCP tests substitute inventory and credentials, so physical validation remains outstanding.
...
- The requested Astra High files/folders audit found and fixed a payload-consent race: source edits after approval could previously cross the network before receiver hash rejection. Sender preparation now stages a private snapshot with descriptor-relative no-follow traversal and mutation checks; the approved SHA256 manifest describes the exact bytes sent. Regression tests inspect transmitted chunks after source mutation. The complete 438-test suite passes. Tracking: pasta-yti.1; integrated into PR #127.
